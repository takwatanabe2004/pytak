...
def get_gridsearch_classifier(clf_name):
    ...
    if clf_name == 'sklLogregL1':
        ...
        raise Exception("ttestRbfSvm not worth running for HARDI")
...
#        param_grid = {  'ttest__k':  (2**np.arange(2,8,2)).astype(int),
#                          'svm__C': 10.**np.arange(-8,13,3),#^^^^^must be int, or scikit will complain
#                      'svm__gamma': 10.**np.arange(-16,-8,2)}
        ...
        param_grid = {'ttest__k':  (2**np.arange(2,7.5,0.5)).astype(int), # must be int, or scikit will complain
                      'liblin__C': 2.**np.arange(-18,6,2),}
    ...
    #%% === PCA stuffs...no interpretability, but see if accuracy improves ====
        ...
        param_grid = {'PCA__n_components':np.array([1,2,5, 10, 20, 50, 100,176])}
    #=== PCA + LINSVM ===
        ...
        param_grid = {'PCA__n_components':np.array([1,2,5, 10, 20, 40, 100,176]),
                      'SVM__C':2.**np.arange(-14,3,2)}
    #%% PCA + RBFSVM
    elif  clf_name == 'PcaRbfSvm':
        """programs hangs at certain rbf bandwidth...skip"""
        raise Exception(clf_name+" not worth running for HARDI")
#        from tak.ml import PrecomputedRBFSVM
#        from sklearn.decomposition import PCA
#        from sklearn.pipeline import Pipeline
#
...
#                      'SVM__gamma': 2.**np.arange(-18,-1,2)}
    #%% ttest + LDA (for interpretability, I guess)
        ...
        param_grid = {'ttest__k':  (2**np.arange(2,7.5,0.5)).astype(int)}
