...
def get_gridsearch_classifier(clf_name):
    ...
    if clf_name == 'sklLogregL1':
        ...
        param_grid = {'ttest__k':  (2**np.arange(2,10.5,0.5)).astype(int), # must be int, or scikit will complain
                      'liblin__C': 2.**np.arange(-18,2,2),}
    ...
    #%% === PCA stuffs...no interpretability, but see if accuracy improves ====
        ...
        param_grid = {'PCA__n_components':np.array([1,2,5, 10, 20, 50, 100,200, 300])}
    #=== PCA + LINSVM ===
        ...
        param_grid = {'PCA__n_components':np.array([1,2,5, 10, 20, 40, 100,200, 300]),
...
#                      'SVM__gamma': 2.**np.arange(-18,-6,2)}
    #%% ttest + LDA (for interpretability, I guess)
        ...
        param_grid = {'ttest__k':  (2**np.arange(2,10.5,0.5)).astype(int)}
